...
const genActionArgs = (action) => {
    ...
    return {
        ...baselineArgs,
        IntegrationUri: fname.arn(
            aws,
            config.functions[action.func].lname,
            action
        )
    }
}
...
const put = async (api, awsActions) => {
    for (const action of Object.values(api.integrations)) {
        await putAction(
            api,
            action,
            awsActions.find(
...
const cleanup = async (api, awsActions) => {
    const unused = awsActions.filter(
        (awsAction) => api.integrations[awsAction.Description] === undefined
    )
